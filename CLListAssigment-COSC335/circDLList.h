...
template<class T>
inline void circDLList<T>::addToHead(T val)
{
...
template<class T>
inline void circDLList<T>::addToTail(T val)
{
	if (head != 0) {
		//Not an empty list
		IntDLLNode<T>* tmp;
...
template<class T>
inline void circDLList<T>::sortInsert(T val)
{
	if (head == 0)
	{
		addToHead(val);
...
template<class T>
inline void circDLList<T>::insert(int pos, T val)
{
